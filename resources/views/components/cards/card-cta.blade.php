...
<div
    ...
    <div class="flex flex-col gap-y-3 sm:gap-y-4 items-center h-full">
        ...
        <p class="font-medium text-center text-sm sm:text-base md:text-lg lg:text-xl text-text-medium group-hover:text-text-light transition-colors duration-300 flex-grow">
            {{ $description }}
        </p>
        ...
        <x-layout.shared.button
            :href="$cta_url"
            variant="secondary"
            class="px-6 sm:px-8 py-2 sm:py-3 text-base sm:text-lg w-full font-bold opacity-0 group-hover:opacity-100 transition-opacity duration-300 mt-auto">
            {{ $cta_label }}
        </x-layout.shared.button>
    </div>
</div>
